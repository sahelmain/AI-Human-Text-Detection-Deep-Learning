...
def load_models():
    ...
    possible_paths = [
        os.path.join(current_dir, 'ai_human_detection_project', 'models'),  # Original structure
        os.path.join(current_dir, 'models'),  # Direct models folder
        'models',  # Relative path
        'ai_human_detection_project/models'  # Relative path with subfolder
    ]
    ...
    if models_dir is None:
        st.error(f"Models directory not found in any of these locations: {possible_paths}")
        return None, None
